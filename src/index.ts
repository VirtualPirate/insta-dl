...
export async function getReelInfo(url: string) {
  // Get the raw HTML
  ...
  const rawInfo = JSON.parse(
    $('script[type="application/ld+json"]').text().toString()
  );
  ...
  const refineInfo = {
    ...
    video: {
      likesCount: rawInfo?.interactionStatistic?.find((item: any) => {
        return item.interactionType.includes("LikeAction");
      }).userInteractionCount,

      commentCount: rawInfo?.interactionStatistic?.find((item: any) => {
